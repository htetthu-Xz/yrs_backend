...
@section('header')
    <div class="tw-flex tw-justify-between tw-items-center">
        <div class="tw-flex tw-justify-between tw-items-center">
            <i class="fas fa-key tw-p-3 tw-bg-white tw-rounded-lg tw-shadow tw-mr-1"></i>
            <h5 class=" tw-text-lg">Change Password</h5>
        </div>
    </div>
@endsection
...
@section('content')
    <x-card>
        <form method="post" id="change-password-form" action="{{ route('change-password.update') }}" class="tw-space-y-6">
            @csrf
            @method('put')
            ...
            <div class="tw-flex tw-items-center tw-justify-center tw-mt-5">
                <x-cancel-button href="{{ route('dashboard') }}" class="tw-mr-2">Cancel</x-cancel-button>
...
@push('scripts')
    {!! JsValidator::formRequest(\App\Http\Requests\ChangePasswordRequest::class, '#change-password-form') !!}
@endpush
